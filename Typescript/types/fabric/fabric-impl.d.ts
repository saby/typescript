...
// Fabric Extensions Start
type AnimationContext = {
    cancel: () => void;
};

type RunningAnimations = AnimationContext[] & {
    findAnimation: Function;
    findAnimationIndex: Function;
    findAnimationsByTarget: Function;
    cancelAll: Function;
    cancelByTarget: Function;
};

export const runningAnimations: RunningAnimations;

interface IUtilDomEvent {
    isTouchEvent(e: IEvent): boolean;
}
...
interface IUtilMisc {
    ...
    loadImage(
        url: string,
        callback: (image: HTMLImageElement, isError: boolean) => void,
        context?: any,
        crossOrigin?: string
    ): void;
}

type ControlsUtilsStyleHandler = (
    eventData: MouseEvent,
    control: Control,
    fabricObject: Object
) => string;
type ControlsUtilsActionHandler = (
    eventData: MouseEvent,
...
interface IControlsUtils {
    scaleCursorStyleHandler: ControlsUtilsStyleHandler;
    skewCursorStyleHandler: ControlsUtilsStyleHandler;
    scaleSkewCursorStyleHandler: ControlsUtilsStyleHandler;
    rotationStyleHandler: ControlsUtilsStyleHandler;
    rotationWithSnapping: ControlsUtilsActionHandler;
    scalingEqually: ControlsUtilsActionHandler;
    scalingX: ControlsUtilsActionHandler;
    scalingY: ControlsUtilsActionHandler;
    scalingYOrSkewingX: ControlsUtilsActionHandler;
    scalingXOrSkewingY: ControlsUtilsActionHandler;
    changeWidth: ControlsUtilsActionHandler;
    skewHandlerX: ControlsUtilsActionHandler;
    skewHandlerY: ControlsUtilsActionHandler;
    dragHandler: ControlsUtilsActionHandler;
    scaleOrSkewActionName: ControlsUtilsStyleHandler;

    fireEvent: Function;
    wrapWithFixedAnchor: Function;
...
export interface ISelectEvent extends fabric.IEvent {
...
export interface IContentSizeEvent extends ICanvasEvent, fabric.IEvent {
    size: { left: number; top: number; height: number; width: number };
    firstRender: boolean;
}

export interface IViewportChangedEvent extends ICanvasEvent, fabric.IEvent {
    isZoomChanged?: boolean;
    /**
     * Изменение пришло из внешнего компонента, например, скроллбара.
     */
    isExternalAction?: boolean;
}

export interface IScrollEvent extends fabric.IEvent {
    deltaX: number;
    deltaY: number;
}

export interface IPinchEvent extends fabric.IEvent {
    startDistance: number;
    distance: number;
    center: fabric.IPoint;
}

export interface IModifiedEvent extends fabric.IEvent<MouseEvent> {
    action: 'drag' | 'scale' | 'scaleX' | 'scaleY' | 'rotate';
}

export interface IBeforeTransformEvent extends fabric.IEvent<MouseEvent> {
    transform: Transform;
}
...
// Canvas Editor Extensions

export interface IHoverOptions {
    hover: fabric.IObjectOptions;
    normal: fabric.IObjectOptions;
    disabled?: boolean;
    triggerOnThisOnly?: boolean;
}
...
export interface ICanvasOptions {
    externalDraggingCursor?: string;
    rightBehaveLeft?: boolean;
}

interface IObservable<T> {
    on(eventName: 'object:modified', handler: (e: IModifiedEvent) => void): T;
    on(eventName: 'canvas:contentSize', handler: (e: IContentSizeEvent) => void): T;
    on(eventName: 'viewport:changed', handler: (e: IViewportChangedEvent) => void): T;
}

class FixedGroup extends fabric.Group {
...
export class Object {
    ...
    protected callSuper(methodName: string, ...args: unknown[]): any;
    /**
...
interface IUtilAnimation {
    ...
    cancelAnimFrame(timeoutId: number): void;
}
